-- Script SQL mis à jour pour Supabase avec gestion des abonnements premium

-- Création de la table profiles avec champs premium
CREATE TABLE IF NOT EXISTS profiles (
  ...
  premium_plan TEXT DEFAULT 'monthly',
...
-- Ajouter les champs premium à la table profiles existante (si ils n'existent pas)
...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.profiles (id, email, full_name, avatar_url, is_premium, premium_plan)
  VALUES (
    NEW.id,
    NEW.email,
    NEW.raw_user_meta_data->>'full_name',
    NEW.raw_user_meta_data->>'avatar_url',
    FALSE, -- Par défaut, pas premium
...
-- Fonction pour vérifier si un utilisateur a un abonnement premium valide
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      ...
      WHEN p.premium_expires_at > NOW() THEN EXTRACT(DAY FROM (p.premium_expires_at - NOW()))::INTEGER
      ELSE 0
